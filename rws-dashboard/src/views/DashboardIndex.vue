<script setup>
import { useDataStore } from '@/stores/dataStore';
import { ref } from 'vue';
import RevenueBarChart from '../components/RevenueBarChart.vue';
import StatusPieChart from '../components/StatusPieChart.vue';
import RevenueLineChart from '../components/RevenueLineChart.vue';
import FilterBar from '../components/FilterBar.vue';

// Get the data store
const store = useDataStore();
const fileInput = ref(null);

// Handle file selection from the file input
const handleFileSelect = (event) => {
  store.processFile(event.target.files[0]);
};

// Handle drag-and-drop upload events
const handleDragOver = () => {
  store.setDragging(true);
};

const handleDragLeave = () => {
  store.setDragging(false);
};

const handleDrop = (event) => {
  store.setDragging(false);
  store.processFile(event.dataTransfer.files[0]);
};

// Clear data when the refresh button is clicked
const refreshData = () => {
  store.refreshData();
  if (fileInput.value) {
    fileInput.value.value = null;
  }
};
</script>

<template>
  <main>
    <h1 class="page-title">Wholesale Dashboard</h1>

    <div class="upload-section">
      <!-- Refresh button -->
      <div class="refresh-upload">
        <button class="btn btn-primary" @click="refreshData">
          <span class="material-icons">refresh</span>
          Refresh Data
        </button>
      </div>
      <!-- File upload zone (Drag & Drop) -->
      <div class="upload-zone" 
           :class="{ 'is-dragging': store.isDragging }" 
           @dragover.prevent="handleDragOver" 
           @dragleave.prevent="handleDragLeave" 
           @drop.prevent="handleDrop">
        <input
          type="file"
          id="file-upload"
          ref="fileInput"
          accept=".json, .xlsx, .xls"
          @change="handleFileSelect"
        />
        <p>Drag & Drop your Excel file here, or <span>click to select</span>.</p>
      </div>

    </div>

    <!-- Using reusable FilterBar component -->
    <FilterBar :showItemsPerPage="false" />
    
    <div class="summary-cards">
      <!-- Total Revenue Card -->
      <div class="card">
        <h2>Total Revenue</h2>
        <p>Rp.{{ store.totalRevenue.toLocaleString() }}</p>
      </div>

      <!-- New Customer Card -->
      <div class="card">
        <h2>New Customers</h2>
        <p>{{ store.newCustomerCount }}</p>
      </div>

      <!-- Modified Orders Card -->
      <div class="card">
        <h2>Modified Orders</h2>
        <p>{{ store.modifiedCustomerCount }}</p>
      </div>

      <!-- Cancelled Order Card -->
      <div class="card">
        <h2>Cancelled Orders</h2>
        <p>{{ store.canceledCustomerCount }}</p>
      </div>
    </div>

    <!-- If data is loading -->
    <div v-if="store.isLoading" class="loading-container">
      <div class="loading-spinner"></div>
      <p>Loading your data...</p>
    </div>

    <!-- If there is no data -->
    <div v-else-if="!store.dashboardData.length" class="empty-state">
      <span class="material-icons">upload_file</span>
      <p>No data available. Please upload an Excel or JSON file.</p>
    </div>

    <template v-else>
      <!-- Add Revenue Line Chart with specific class -->
      <div class="chart-container line-chart">
        <h3>Revenue Trends by Period</h3>
        <RevenueLineChart :chart-data="store.revenueLineChartData" />
      </div>

      <div class="charts-wrapper">
        <div class="chart-container">
          <h3>Revenue by Customer</h3>
          <RevenueBarChart :chart-data="store.revenueChartData" />
        </div>
        <div class="chart-container">
          <h3>Order Status Breakdown</h3>
          <StatusPieChart :chart-data="store.pieChartData" />
        </div>
      </div>
    </template>
  </main>
</template>